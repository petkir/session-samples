    ...
    if [[ "$upload_choice" =~ ^[Yy]$ ]]; then
        ...
        if ! curl -s http://localhost:6333/collections >/dev/null 2>&1; then
            ...
            if ! command_exists docker; then
                ...
                print_status "You can install Docker from: https://docs.docker.com/desktop/install/mac-install/"
                exit 1
            fi
        ...
        if [ -d "elat.local.llm/elat.local.llm.qdrant.init" ]; then
            cd elat.local.llm/elat.local.llm.qdrant.init
