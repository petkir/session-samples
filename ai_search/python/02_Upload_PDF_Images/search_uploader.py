"""
Enhanced Azure AI Search uploader module for creating search indices and uploading documents.
Handles index creation, document upload, and error handling for both text and image content.
"""
import asyncio
...
class SearchUploader:
    """Handles uploading documents to Azure AI Search with support for both text and image content."""
    ...
    async def create_or_update_index(self) -> bool:
        """
        Create or update the search index with proper schema for text and image content.
        ...
        try:
            ...
            # Define index fields (enhanced for image content)
            fields = [
                ...
                SearchableField(name="objects_detected", type=SearchFieldDataType.Collection(SearchFieldDataType.String)),
                SimpleField(name="confidence_score", type=SearchFieldDataType.Double, filterable=True),
                SimpleField(name="analysis_timestamp", type=SearchFieldDataType.DateTimeOffset, filterable=True),
                SimpleField(name="tokens_used", type=SearchFieldDataType.Int32, filterable=True)
            ]
    ...
    async def upload_documents(self, chunks: List[DocumentChunk], embeddings: List[EmbeddingResult]) -> Dict[str, Any]:
        """
        Upload documents to the search index (supports both text and image content).
        ...
        for chunk in chunks:
            ...
            if chunk.content_type == "image":
                # Extract image-specific data from the chunk ID or content
                document.update({
                    "image_path": None,  # Will be set if available
                    "objects_detected": [],
                    "confidence_score": 0.0,
                    "analysis_timestamp": formatted_date,
                    "tokens_used": 0
                })
            else:
                # Set default values for text content
                document.update({
                    "image_path": None,
                    "objects_detected": [],
                    "confidence_score": 0.0,  # Default to 0.0 instead of None
                    "analysis_timestamp": formatted_date,  # Use creation time instead of None
                    "tokens_used": 0  # Default to 0 instead of None
                })
    ...
    async def upload_image_analyses(self, image_analyses: List[ImageAnalysis], embeddings: List[EmbeddingResult]) -> Dict[str, Any]:
        """
        Upload image analyses directly to the search index.
        ...
        Returns:
            Dictionary with upload results
        """
        if not image_analyses or not embeddings:
            logger.warning("No image analyses to upload")
            return {"success": 0, "failed": 0, "errors": []}
        
        logger.info("Uploading image analyses to search index", 
                   analysis_count=len(image_analyses), 
                   embedding_count=len(embeddings))
        
        # Create mapping of image_id to embedding
        embedding_map = {result.chunk_id: result for result in embeddings}
        ...
        for analysis in image_analyses:
            embedding_result = embedding_map.get(analysis.image_id)
            
            if not embedding_result or not embedding_result.success:
                logger.warning("Skipping image analysis due to missing/failed embedding", 
            ...
            # Format datetime for Azure AI Search
            formatted_date = analysis.analysis_timestamp.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
            ...
            document = {
                ...
                "headline": headline,
                "embedding": embedding_result.embedding,
                "file_name": analysis.pdf_source.split('/')[-1],  # Extract filename
                ...
                "objects_detected": analysis.objects_detected if isinstance(analysis.objects_detected, list) else [],
        ...
        if not documents:
            logger.error("No valid image analyses to upload")
            return {"success": 0, "failed": len(image_analyses), "errors": ["No valid image analyses"]}
        ...
        logger.info("Image analysis upload completed", 
    ...
    async def search_documents(self, query: str, top: int = 10, content_type: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Search documents in the index with optional content type filtering.
        ...
        """
        try:
            logger.info("Searching documents", query=query, top=top, content_type=content_type)
            
            # Build filter if content type is specified
            filter_expression = None
            if content_type:
                filter_expression = f"content_type eq '{content_type}'"
