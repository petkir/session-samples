...
class Settings(BaseSettings):
    ...
    azure_search_api_version: str = Field(default="2025-05-01-preview", description="Azure AI Search API version")
    ...
    entra_group_id: Optional[str] = Field(default=None, description="Entra Group ID for document access control")
    enable_permission_filter: bool = Field(default=True, description="Enable permission-based filtering")
    ...
    @validator('entra_group_id')
    def validate_entra_group_id(cls, v, values):
        """Validate that Entra Group ID is provided when permission filter is enabled."""
        enable_permission_filter = values.get('enable_permission_filter', False)
        if enable_permission_filter and not v:
            raise ValueError("entra_group_id is required when enable_permission_filter is True")
        return v
