...
# and may not be available in all SDK versions
...
class SearchUploader:
    ...
    def _get_document_acl_groups(self) -> List[str]:
        """
        Get the ACL groups that should have access to the documents.
        
        Returns:
            List of group IDs that should have access to the documents
        """
        if not settings.enable_permission_filter:
            return ["all"]  # Public access when permission filtering is disabled
        
        return [settings.entra_group_id]
    ...
    async def create_or_update_index(self) -> bool:
        ...
        """
        try:
            logger.info("Creating or updating search index with ACL support", 
                       index_name=self.index_name, 
                       permission_filter_enabled=settings.enable_permission_filter)
            ...
            fields = [
                ...
                # ACL fields for access control
                SearchField(
                    name="groupIds", 
                    type=SearchFieldDataType.Collection(SearchFieldDataType.String),
                    filterable=True,
                    # Using string literal for preview feature compatibility
                    **{"permissionFilter": "groupIds"} if settings.enable_permission_filter else {}
                )
            ]
            ...
            # Enable permission filtering if configured (preview feature)
            if settings.enable_permission_filter:
                # Use string literal for preview feature compatibility
                index.permission_filter_option = "enabled"
            
            # Create or update index
            result = await self.index_client.create_or_update_index(index)
            
            logger.info("Index created/updated successfully with ACL support", 
                       index_name=self.index_name, 
                       field_count=len(fields),
                       permission_filter_enabled=settings.enable_permission_filter,
                       entra_group_id=settings.entra_group_id if settings.enable_permission_filter else None)
    ...
    async def upload_documents(self, chunks: List[DocumentChunk], embeddings: List[EmbeddingResult]) -> Dict[str, Any]:
        ...
        logger.info("Uploading documents to search index with ACL support", 
                   chunk_count=len(chunks), 
                   embedding_count=len(embeddings),
                   permission_filter_enabled=settings.enable_permission_filter,
                   acl_groups=self._get_document_acl_groups())
        ...
        for chunk in chunks:
            ...
            document = {
                ...
                # Add ACL metadata for access control
                "groupIds": self._get_document_acl_groups()
            }
